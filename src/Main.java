...
//        if(!tigerFile.exists() || tigerFile.length() == 0){
//            return;
//        }
    ...
    public static void writeObjectsToFile(Tiger tiger, Penguin penguin, Dolphin dolphin) {
        try (
            ObjectOutputStream tigerObject = new ObjectOutputStream(new FileOutputStream("tiger.txt"));
            ObjectOutputStream penguinObject = new ObjectOutputStream(new FileOutputStream("penguin.txt"));
            ObjectOutputStream dolphinObject = new ObjectOutputStream(new FileOutputStream("dolphin.txt"));
        ){
            tigerObject.writeObject(tiger);
            penguinObject.writeObject(penguin);
            dolphinObject.writeObject(dolphin);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void readObjectsFromFile(){
        try (
                BufferedReader tigerIn = new BufferedReader(new FileReader("tiger.txt"));
                BufferedReader penguinIn = new BufferedReader(new FileReader("penguin.txt"));
                BufferedReader dolphinIn = new BufferedReader(new FileReader("dolphin.txt"))
        ){
            String line;
            while((line = tigerIn.readLine()) != null){
                System.out.println(line);
            }
            while((line = penguinIn.readLine()) != null){
                System.out.println(line);
            }
            while((line = dolphinIn.readLine()) != null){
                System.out.println(line);
            }
        }catch (IOException e){
